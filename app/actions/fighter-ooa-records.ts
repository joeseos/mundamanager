...
async function fetchInjuredSnapshotMap(
  ...
  const [{ data: fighters, error: fightersError }, { data: vehicles, error: vehiclesError }] = await Promise.all([
    supabase
      .from('fighters')
      .select('id, fighter_name, fighter_type, fighter_class, fighter_classes, gang_id, gangs!gang_id(id, name)')
      .in('id', ids),
    supabase
      .from('vehicles')
  ...
  (fighters || []).forEach((f: any) => {
    ...
    map.set(f.id, {
      ...
      injured_fighter_class: f.fighter_classes?.join(', ') || f.fighter_class ?? null,
...
export async function createFighterOoaRecord(params: {
  ...
  try {
    ...
    const { data: causing, error: causingError } = await supabase
      .from('fighters')
      .select('id, fighter_name, fighter_type, fighter_class, fighter_classes, gang_id, gangs!gang_id(name)')
    ...
    const row: Record<string, unknown> = {
      ...
      causing_fighter_class: causing.fighter_classes?.join(', ') || causing.fighter_class ?? null,
