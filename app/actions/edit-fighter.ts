...
export async function updateFighterXpWithOoa(params: UpdateFighterXpWithOoaParams): Promise<EditFighterResult> {
  try {
    ...
    const { data: fighter, error: fighterError } = await supabase
      .from('fighters')
      .select('id, gang_id, xp, kills, kill_count, fighter_name, fighter_type, fighter_class, fighter_classes, fighter_type_id, fighter_types(is_spyrer), gangs!gang_id(name)')
      .eq('id', params.fighter_id)
      .single();
    ...
    if (params.ooa_records && params.ooa_records.length > 0) {
      try {
        await insertFighterOoaRecords(supabase, {
          ...
          causing_fighter_class: fighter.fighter_classes?.join(', ') || fighter.fighter_class,
...
export async function updateFighterDetails(params: UpdateFighterDetailsParams): Promise<EditFighterResult> {
  try {
    ...
    if (params.fighter_class !== undefined) {
      ...
      updateData.fighter_classes = params.fighter_classes ?? [params.fighter_class];
    }
    ...
    try {
      ...
      if (params.kill_count !== undefined && previousKillCount !== updatedFighter.kill_count) {
        await logFighterAction({
          gang_id: fighter.gang_id,
          fighter_id: params.fighter_id,
          fighter_name: updatedFighter.fighter_name,
          action_type: 'fighter_kill_count_changed',
          old_value: previousKillCount,
          new_value: updatedFighter.kill_count,
          user_id: user.id
        });
      }
