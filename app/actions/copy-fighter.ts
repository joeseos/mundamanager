...
export async function copyFighter(params: CopyFighterParams): Promise<CopyFighterResult> {
  try {
    ...
    const fighterData: any = {
      ...
      fighter_classes: sourceFighter.fighter_classes || (sourceFighter.fighter_class ? [sourceFighter.fighter_class] : []),
      ...
      for (const beastRecord of sourceFighter.fighter_exotic_beasts) {
        ...
        const { data: newBeastFighter, error: beastInsertError } = await supabase
          ...
          .insert({
            ...
            fighter_classes: beastFighter.fighter_classes || (beastFighter.fighter_class ? [beastFighter.fighter_class] : []),
    ...
    if (copiedVehicleCount > 0) {
      revalidateTag(CACHE_TAGS.BASE_GANG_VEHICLES(params.target_gang_id), { expire: 0 });
    }
