...
interface AdminFighterTradingPostProps {
  ...
  setTradingPostEquipment: (
    equipment: string[] | ((prev: string[]) => string[])
  ) => void;
  disabled?: boolean;
}
...
export function AdminFighterTradingPost({
  ...
  const handleSave = () => {
    toast({
      description:
        'Trading Post options saved. Remember to update the fighter type to apply changes.',
      variant: 'default',
    });
    return true;
  };
  ...
  const modalContent = (
    <div className="space-y-4">
      <div className="flex items-center gap-2 mb-2">
        ...
        />
        <label
          htmlFor="exclude-exclusive-checkbox"
      ...
      <p className="text-sm text-gray-500">
        Select equipment items that should be available in the Trading Post for
        this fighter type.
      </p>
            ...
            Object.entries(equipmentByCategory)
              ...
              .map(([category, items]) => {
                ...
                const checkedCount = relevantIds.filter((id) =>
                  tradingPostEquipment.includes(id)
                ).length;
                const allChecked =
                  checkedCount === relevantIds.length && relevantIds.length > 0;
                const someChecked =
                  checkedCount > 0 && checkedCount < relevantIds.length;
                      ...
                      <div className="flex items-center">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) => {
                            ...
                            const nonExclusiveIds = items
                              .filter((item) => item.availability !== 'E')
                              .map((item) => item.id);
                            if (checked) {
                              setTradingPostEquipment((prev: string[]) => {
                                if (excludeExclusive) {
                                  return Array.from(
                                    new Set([...prev, ...nonExclusiveIds])
                                  );
                                } else {
                                  return Array.from(
