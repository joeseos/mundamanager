...
export default function CampaignTerritoryList({
  ...
  const [territoryToDelete, setTerritoryToDelete] = useState<{
    id: string;
    name: string;
  } | null>(null);
  ...
  const getGangDetails = (gangId: string) => {
    ...
    for (const member of members) {
      const gang = member.gangs.find(
        (g: Member['gangs'][0]) => g.gang_id === gangId
      );
      if (gang) {
        return {
          id: gang.gang_id,
                  ...
                  <tr key={territory.id} className="border-b last:border-0">
                    ...
                    <td className="w-3/5 px-4 py-2">
                      <div className="flex items-center gap-2 flex-wrap">
                        {territory.owning_gangs &&
                        territory.owning_gangs.length > 0 ? (
      ...
      {showTerritoryEditModal && territoryToEdit && (
        <TerritoryEditModal
          ...
          currentDefaultGangTerritory={
            territoryToEdit.default_gang_territory || false
          }
        />
      )}
