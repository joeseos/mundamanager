...
import {
  ...
  vehicleCompatibleCategories,
} from '@/utils/vehicleEquipmentCategories';
...
export default function GangInventory({
  ...
  const handleMoveToFighter = async () => {
    ...
    try {
      ...
      const targetId = isVehicleTarget
        ? selectedFighter.replace('vehicle-', '')
        : selectedFighter;
      ...
      for (const itemIndex of selectedItems) {
        ...
        if (!result.success) {
          console.error(
            `Failed to move item ${stashItem.equipment_name || stashItem.vehicle_name}: ${result.error}`
          );
          errorCount++;
          continue;
        }
        ...
        if (isVehicleTarget) {
          ...
          if (targetVehicle) {
            ...
            const newEquipment: Equipment & Partial<VehicleEquipment> = {
              ...
              vehicle_weapon_id:
                stashItem.equipment_type === 'weapon'
                  ? responseData?.equipment_id || stashItem.id
                  : undefined,
            ...
            if (crewFighter) {
              ...
              const updatedCrewFighter: FighterProps = {
                ...
                vehicles: crewFighter.vehicles?.map((v) =>
                  v.id === targetId
                    ? ({ ...v, equipment: updatedVehicle.equipment } as Vehicle)
                    : v
                ),
              };
              ...
              setFighters((prev) =>
                prev.map((f) =>
                  f.id === crewFighter.id ? updatedCrewFighter : f
                )
              );
          ...
          const currentFighter: FighterProps | undefined =
            updatedFighter || fighters.find((f) => f.id === targetId);
          if (currentFighter) {
            ...
            updatedFighter = {
              ...
              weapons:
                stashItem.equipment_type === 'weapon'
                  ...
                  : currentFighter.weapons || [],
              wargear:
                stashItem.equipment_type === 'wargear'
                  ? [
      ...
      toast({
        ...
        description:
          error instanceof Error
            ? error.message
            : 'Failed to move items from stash',
        variant: 'destructive',
      });
    } finally {
      setIsLoading(false);
  ...
  // Check if any selected item is a vehicle
  const hasSelectedVehicle = selectedItems.some((index) =>
    isVehicle(stash[index])
  );
          ...
          {stash.length === 0 ? (
            ...
            <>
                ...
                <div className="flex justify-end mb-2 pr-2">
                  <span className="text-sm font-normal">
                    ...
                    {selectedItems.reduce(
                      (sum, index) => sum + (stash[index].cost || 0),
                      0
                    )}
                  </span>
                </div>
              ...
              <div className="flex justify-end mb-2 pr-2">
                <span className="text-sm font-normal">
                  Total Value:{' '}
                  {stash.reduce((sum, item) => sum + (item.cost || 0), 0)}
                </span>
              </div>
              ...
              <div className="px-0">
                <div className="border-t pt-4">
                  <label
                    htmlFor="fighter-select"
                    className="block text-sm font-medium text-gray-700 mb-2"
                  >
                    Select Fighter or Vehicle
                    {hasSelectedVehicle && (
                      <span className="text-sm text-gray-500 ml-2">
                        (Only Crew fighters can receive vehicles)
                      </span>
                    )}
                  </label>
                  <select
                    ...
                    <option value="">
                      {selectedItems.length > 0 &&
                        (hasVehicleExclusiveItem
                          ? 'Select a vehicle'
                          : hasSelectedVehicle ||
                              selectedItems.some((index) =>
                                isVehicleCompatible(stash[index])
                              )
                        ...
                        {!hasVehicleExclusiveItem && (
                          <optgroup label="Fighters">
                            {fighters.map((fighter) => {
                              const isDisabled =
                                hasSelectedVehicle && !isCrew(fighter);
                              ...
                              return (
                                ...
                                >
                                  ...
                                  {fighter.fighter_class}) - {fighter.credits}{' '}
                                  credits
                                </option>
                              );
                            })}
        ...
        <ItemModal
          ...
          onEquipmentBought={(
            ...
            const newStashItem: StashItem = {
              ...
              custom_equipment_id: boughtEquipment.is_custom
                ? boughtEquipment.equipment_id
                : undefined,
            };
