...
function breakdownRowKey(
  fighter: { fighter_name: string; fighter_type: string; fighter_class: string; fighter_classes?: string[] },
  index: number,
) {
  const classKey = fighter.fighter_classes?.join(',') || fighter.fighter_class;
...
export default function GangTerritories({ gangId, campaigns = [] }: GangTerritoriesProps) {
  ...
  const { data: fighterStats } = useQuery({
    ...
    queryFn: async () => {
      ...
      return response.json() as Promise<{
        ...
        ooa_breakdown?: Array<{ fighter_name: string; fighter_type: string; fighter_class: string; fighter_classes?: string[]; kills: number }>;
        deaths_breakdown?: Array<{ fighter_name: string; fighter_type: string; fighter_class: string; fighter_classes?: string[] }>;
      }>;
    },
    staleTime: 5 * 60 * 1000,
    gcTime: 10 * 60 * 1000,
  ...
  const ooaBreakdown = useMemo(() => {
    ...
    return [...breakdown].sort(
      (a, b) => getClassRank(a.fighter_classes?.[0] || a.fighter_class) - getClassRank(b.fighter_classes?.[0] || b.fighter_class)
    );
              ...
              {ooaBreakdown.map((fighter, index) => (
                <div key={breakdownRowKey(fighter, index)} className="flex justify-between gap-3">
                  <span className="flex-1 truncate text-left">
                    {fighter.fighter_name} - {fighter.fighter_type} ({fighter.fighter_classes?.join(', ') || fighter.fighter_class})
                  </span>
