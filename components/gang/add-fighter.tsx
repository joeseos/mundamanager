...
export default function AddFighter({
    ...
    if (availableSubTypes.length > 0 && !selectedSubTypeId) {
      ...
      if (defaultSubType) {
        ...
        const cheapestSubType = availableSubTypes.reduce((lowest, current) => {
          const lowestCost =
            fighterTypes.find((ft) => ft.id === lowest.id)?.total_cost ??
            Infinity;
          const currentCost =
            fighterTypes.find((ft) => ft.id === current.id)?.total_cost ??
            Infinity;
  ...
  const handleFighterTypeChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (typeId) {
      ...
      const fighterTypeGroup = fighterTypes.filter(
        (t) =>
  ...
  const renderEquipmentSelection = () => {
    const selectedType = fighterTypes.find(
      (t) => t.id === selectedFighterTypeId
    );
  ...
  const handleAddFighter = async () => {
    ...
    try {
      ...
      const fighterDisplayCost = useBaseCostForRating
        ? actualBaseCost
        : enteredCost;

      // Prepare default equipment from the selected fighter type
      const fighterTypeForEquipment = fighterTypes.find(
        (t) => t.id === fighterTypeIdToUse
      );
      const defaultEquipment =
        fighterTypeForEquipment?.default_equipment?.map((item) => ({
        ...
        >
          ...
          {(() => {
            ...
            return Array.from(typeClassMap.values())
              .sort((a, b) => {
                const classRankA =
                  fighterClassRank[a.fighter.fighter_class.toLowerCase()] ??
                  Infinity;
                const classRankB =
                  fighterClassRank[b.fighter.fighter_class.toLowerCase()] ??
                  Infinity;
          ...
          >
            ...
            {[...availableSubTypes]
              .sort((a, b) => {
                ...
                const aCost =
                  fighterTypes.find((ft) => ft.id === a.id)?.total_cost ?? 0;
                const bCost =
                  fighterTypes.find((ft) => ft.id === b.id)?.total_cost ?? 0;
              ...
              .map((subType) => {
                const subTypeCost =
                  fighterTypes.find((ft) => ft.id === subType.id)?.total_cost ??
                  0;
                const lowestSubTypeCost = Math.min(
                  ...availableSubTypes.map(
                    (sub) =>
                      fighterTypes.find((ft) => ft.id === sub.id)?.total_cost ??
                      Infinity
                  )
                );
