...
--      copy_custom_pack() function defined below.
--
...
-- -----------------------------------------------------------------------------
-- 3. copy_custom_pack(p_pack_id) — atomic deep-clone of a pack + all its items
--    into the calling user's account, with all internal FKs remapped.
--    SECURITY INVOKER: open-SELECT RLS reads the source; owner-INSERT RLS
--    accepts the clones (user_id = auth.uid()). Runs in one transaction.
-- -----------------------------------------------------------------------------

-- Implemented with plpgsql array variables + jsonb id-maps (no temp tables, so
-- the body compiles cleanly under check_function_bodies and avoids cached-plan
-- pitfalls). Maps are jsonb objects keyed by old uuid (text) -> new uuid (text).
...
GRANT EXECUTE ON FUNCTION public.copy_custom_pack(uuid) TO authenticated;
