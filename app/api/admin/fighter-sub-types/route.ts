...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (
      !body.sub_type_name ||
      typeof body.sub_type_name !== 'string' ||
      body.sub_type_name.trim() === ''
    ) {
      return NextResponse.json(
        { error: 'Sub-type name is required' },
        { status: 400 }
      );
    }
    ...
    const formattedName =
      body.sub_type_name.trim().charAt(0).toUpperCase() +
      body.sub_type_name.trim().slice(1);
    ...
    if (searchError) {
      console.error(
        'Database error checking for existing sub-types:',
        searchError
      );
      return NextResponse.json(
        {
    ...
    if (existingSubTypes && existingSubTypes.length > 0) {
      console.log(
        'Found existing sub-type with same name:',
        existingSubTypes[0]
      );
      return NextResponse.json({
    ...
    const { data: newSubType, error } = await supabase
      .from('fighter_sub_types')
      .insert([{ sub_type_name: formattedName }])
      .select('id, sub_type_name')
      .single();
