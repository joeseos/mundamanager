...
export async function GET(request: Request) {
  try {
    ...
    // Optional: attaches each gang's fighters (id, name, type, subtype) for
    ...
    const { data: fighters, error: fightersError } = await supabase
      .from('fighters')
      .select('id, fighter_name, fighter_type, fighter_subtypes, gang_id')
      .in('gang_id', gangIds)
    ...
    const fightersByGang = new Map<string, Array<{ id: string; fighter_name: string; fighter_type: string | null; fighter_subtypes: string[] | null; gang_id: string }>>();
    (fighters || []).forEach((f: any) => {
      ...
      fightersByGang.get(f.gang_id)!.push({
        ...
        fighter_subtypes: f.fighter_subtypes ?? null,
        gang_id: f.gang_id,
      });
    });
