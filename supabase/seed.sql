...
-- unique per edition.
INSERT INTO public.fighter_subtypes (id, subtype_name, created_at) VALUES
...
-- Migrations backfill existing subtypes onto n23, but that runs before this file,
-- so rows seeded above would keep a NULL edition_id — invisible to every
-- edition-scoped query and outside the (edition_id, subtype_name) unique index.
-- Put them on the same edition the backfill uses.
UPDATE public.fighter_subtypes
...
-- left edition-less: fighter_subtypes holds one row per subtype per edition. The
...
SELECT v.subtype_name, e.id
FROM public.editions e
CROSS JOIN (VALUES ('Beast'), ('Pet')) AS v(subtype_name)
WHERE e.slug = 'n26'
  AND NOT EXISTS (
...
INSERT INTO public.fighter_types (id, gang_type_id, gang_type, fighter_type, cost, movement, weapon_skill, ballistic_skill, strength, toughness, wounds, initiative, leadership, cool, willpower, intelligence, attacks, fighter_subtypes, free_skill, is_gang_addition, created_at) VALUES
