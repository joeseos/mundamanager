  ...
  async function fetchUserCampaigns(): Promise<Campaign[]> {
    ...
    try {
      ...
      const { data: campaigns, error: campaignsError } = await supabase
        .from('campaigns')
        .select(
          'id, campaign_name, campaign_type_id, created_at, updated_at, image_url'
        )
        .in('id', campaignIds);
      ...
      // Get campaign types
      const campaignTypeIds = Array.from(
        new Set(campaigns.map((c) => c.campaign_type_id))
      );
      ...
      const campaignsWithDetails = campaigns.map((campaign: any) => {
        const memberData = campaignMembers.find(
          (member) => member.campaign_id === campaign.id
        );
        const typeData = campaignTypes?.find(
          (type) => type.id === campaign.campaign_type_id
        );
      ...
      const userGangsByCampaign: Record<
        string,
      ...
      const sortedCampaigns = campaignsWithGangs.sort(
        (a, b) =>
