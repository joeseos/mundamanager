...
export function DraggableFighters({
  ...
  const [currentPositions, setCurrentPositions] =
    useState<Record<number, string>>(initialPositions);
  ...
  useEffect(() => {
    ...
    const unpositionedFighters = fighters.filter(
      (f) => !Object.values(currentPositions).includes(f.id)
    );
    ...
    if (unpositionedFighters.length > 0) {
      ...
      console.log(
        'Added positions for unpositioned fighters:',
  ...
  const handleDragEnd = async (event: any) => {
    ...
    try {
      ...
      const newPositions = newSortedIds.reduce(
