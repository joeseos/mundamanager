...
export function AdminEditSkillModal({
  onClose,
  onSubmit,
}: AdminEditSkillModalProps) {
  const [skillName, setSkillName] = useState('');
  const [skillId, setSkillId] = useState('');
  const [skillNameList, setSkillList] = useState<
  ...
  const handleSubmitType = async (operation: OperationType) => {
    ...
    setIsLoading(true);
    try {
      const url = '/api/admin/skill-types';
      let method: string;
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to ${operation === OperationType.POST ? 'create' : operation === OperationType.UPDATE ? 'update' : 'delete'} skill`
        );
      }
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  const selectedSkillType =
                    selectedOption.getAttribute('data-skill-type') || '';
                  ...
                  <optgroup key={groupLabel} label={groupLabel}>
                    {skillList.map((type) => (
                      <option
                        key={type.id}
                        value={type.id}
                        data-skill-type={type.skill_type}
                      >
                        {type.skill_type}
                      </option>
                    ))}
