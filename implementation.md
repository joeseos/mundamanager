  ...
  const fighters = await Promise.all(
    fighterIds.map(async (fighterId) => {
      ...
      return {
        ...
        // CRITICAL: Transform equipment into weapons and wargear arrays
        // to match FighterProps interface expected by FighterCard component
        weapons: equipment
...
### 6. Equipment Data Structure Mismatch ⚠️ CRITICAL
**Issue**: The `FighterCard` component expects `weapons: WeaponProps[]` and `wargear: WargearItem[]`, but `getFighterEquipment()` returns a generic `equipment: FighterEquipment[]` array.

**Symptoms**:
- Fighter cards on gang page show no equipment/weapons
- Initial deployment had to be reverted due to this bug
- Type mismatch between data layer and presentation layer

**Root Cause**:
```typescript
// getFighterEquipment() returns:
equipment: FighterEquipment[] = [
  { equipment_type: 'weapon', equipment_name: 'Lasgun', ... },
  { equipment_type: 'wargear', equipment_name: 'Armor', ... }
]

// But FighterCard expects:
interface FighterCardProps {
  weapons: WeaponProps[];  // ❌ Doesn't exist in data
  wargear: WargearItem[];  // ❌ Doesn't exist in data
}
```

**Solution**:
Transform equipment array into weapons and wargear arrays in `getGangFightersList()` to match `FighterProps` interface:

```typescript
// In getGangFightersList() return statement:
return {
  // ... other fighter properties ...

  // Transform equipment into separate arrays
  weapons: equipment
    .filter((item: any) => item.equipment_type === 'weapon')
    .map((item: any) => ({
      fighter_weapon_id: item.fighter_equipment_id,
...
1. Both gang page and fighter page must return the same data structure
2. The `FighterProps` interface is the contract between data and UI
3. Without this transformation, equipment simply doesn't display
4. This was the bug that caused the production revert

**Type Safety**:
Update `GangFighter` interface in `gang-data.ts`:
```typescript
import { WeaponProps, WargearItem } from '@/types/fighter';

export interface GangFighter {
  // ... other properties ...
  weapons: WeaponProps[];   // ✅ Changed from equipment: any[]
  wargear: WargearItem[];   // ✅ Added
  // ... rest of properties ...
}
```

**Verification**:
- ✅ Gang page fighter cards show weapons and wargear
- ✅ Fighter page fighter cards show weapons and wargear
- ✅ Both pages use same data structure from shared functions
