...
interface FighterXpModalProps {
  ...
  xpAmountState: { xpAmount: string; xpError: string };
...
export function FighterXpModal({
  ...
  const handleXpCountChange = (id: string, value: number) => {
    setXpCounts((prev) => ({ ...prev, [id]: value }));
  };
  ...
  useEffect(() => {
    ...
    const calculatedAmount =
      totalXpFromCountsAndCheckboxes === 0
        ? ''
        : String(totalXpFromCountsAndCheckboxes);
  ...
  return (
    <Modal
      ...
      content={
        <div className="space-y-4">
                ...
                <div className="flex items-center space-x-2">
                  <Button
                    ...
                    onClick={() =>
                      handleXpCountChange(
                        xpCase.id,
                        Math.max(0, xpCounts[xpCase.id] - 1)
                      )
                    }
                  >
                  ...
                  <Button
                    ...
                    className="flex items-center justify-center border bg-background hover:bg-accent hover:text-accent-foreground h-10 w-10 rounded-md"
                    onClick={() =>
                      handleXpCountChange(xpCase.id, xpCounts[xpCase.id] + 1)
                    }
                  >
          ...
          <div className="text-xs text-gray-600">
            ...
            <div>
              Below value can be overridden (use a negative value to subtract)
            </div>
          </div>
      ...
      onConfirm={onConfirm}
      confirmText={
        parseInt(xpAmountState.xpAmount || '0', 10) < 0
          ? 'Subtract XP'
          : 'Add XP'
      }
      confirmDisabled={
        !xpAmountState.xpAmount || !isValidXpInput(xpAmountState.xpAmount)
      }
    />
  );
}
