...
type Fighter = {
  ...
  fighter_effects?: Array<{
    type_specific_data: { credits_increase?: number };
  }>;
  vehicles?: Array<{
    id: string;
    cost: number;
...
export const getUserGangs = cache(async function fetchUserGangs(): Promise<
  Gang[]
> {
  ...
  try {
    ...
    const gangsWithRatings = await Promise.all(
      data.map(async (gang: any) => {
        try {
          const fighters = await getFightersWithRating(supabase, gang.id);
          const totalRating = fighters.reduce(
            (sum: number, fighter: FighterWithRating) => sum + fighter.rating,
            0
          );
    ...
    console.log(
      `Server: Processed ${gangsWithRatings.length} gangs with ratings`
    );
...
async function getFightersWithRating(
  ...
  try {
    ...
    if (vehiclesError) {
      console.error(
        `Error fetching vehicles data for gang ${gangId}:`,
        vehiclesError
      );
      throw vehiclesError;
    }
    ...
    return (fighters as Fighter[]).map((fighter) => {
      try {
        ...
        if (fighter.fighter_equipment) {
          const equipmentCost = fighter.fighter_equipment.reduce(
            (sum: number, eq: { purchase_cost: number }) =>
              sum + (eq.purchase_cost || 0),
            0
          );
          rating += equipmentCost;
          console.log(
            `Fighter ${fighter.id}: equipment cost = ${equipmentCost}`
          );
        }

        // Add skills costs
        if (fighter.fighter_skills) {
          const skillsCost = fighter.fighter_skills.reduce(
            (sum: number, skill: { credits_increase: number }) =>
              sum + (skill.credits_increase || 0),
            0
          );
          rating += skillsCost;
        ...
        if (fighter.fighter_effects) {
          const effectsCost = fighter.fighter_effects.reduce(
            (
              sum: number,
        ...
        fighterVehicles.forEach((vehicle) => {
          ...
          if (vehicle.fighter_equipment) {
            vehicleCost += vehicle.fighter_equipment.reduce(
              (sum: number, eq: { purchase_cost: number }) =>
                sum + (eq.purchase_cost || 0),
              0
            );
          ...
          if (vehicle.fighter_effects) {
            vehicleCost += vehicle.fighter_effects.reduce(
              (
                sum: number,
                effect: { type_specific_data: { credits_increase?: number } }
              ) => {
        ...
        rating += vehicleCost;
        console.log(
          `Fighter ${fighter.id}: vehicle cost = ${vehicleCost}, total rating = ${rating}`
        );

        return { id: fighter.id, rating };
      } catch (calculationError) {
        console.error(
          `Error calculating rating for fighter ${fighter.id}:`,
          calculationError
        );
    ...
    console.error(
      `Unexpected error in getFightersWithRating for gang ${gangId}:`,
      error
    );
    return [];
  }
