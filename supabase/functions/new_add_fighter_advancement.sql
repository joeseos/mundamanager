...
DROP FUNCTION IF EXISTS public.new_add_fighter_advancement(UUID, UUID, INTEGER, INTEGER, UUID);
DROP FUNCTION IF EXISTS public.new_add_fighter_advancement(UUID, UUID, INTEGER, INTEGER);

-- Create function with proper XP cost handling
CREATE OR REPLACE FUNCTION public.new_add_fighter_advancement(
...
REVOKE ALL ON FUNCTION public.new_add_fighter_advancement(UUID, UUID, INTEGER, INTEGER) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.new_add_fighter_advancement(UUID, UUID, INTEGER, INTEGER) TO authenticated;
GRANT EXECUTE ON FUNCTION public.new_add_fighter_advancement(UUID, UUID, INTEGER, INTEGER) TO service_role;