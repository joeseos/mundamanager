...
async function fetchInjuredSnapshotMap(
  ...
  const [{ data: fighters, error: fightersError }, { data: vehicles, error: vehiclesError }] = await Promise.all([
    supabase
      .from('fighters')
      .select('id, fighter_name, fighter_type, fighter_subtypes, gang_id, gangs!gang_id(id, name)')
      .in('id', ids),
    supabase
      .from('vehicles')
  ...
  (fighters || []).forEach((f: any) => {
    ...
    map.set(f.id, {
      ...
      injured_fighter_subtypes: f.fighter_subtypes || [],
 ...
 * Snapshots the injured fighter's name, type, subtype, gang name, and (for
...
export async function insertFighterOoaRecords(
  ...
  params: {
    ...
    causing_fighter_subtypes?: string[] | null;
  ...
  const rows = validRecords.map(r => {
    ...
    return {
      ...
      causing_fighter_subtypes: params.causing_fighter_subtypes ?? [],
      ...
      injured_fighter_subtypes: snapshot?.injured_fighter_subtypes ?? [],
...
export async function createFighterOoaRecord(params: {
  ...
  try {
    ...
    const { data: causing, error: causingError } = await supabase
      .from('fighters')
      .select('id, fighter_name, fighter_type, fighter_subtypes, gang_id, gangs!gang_id(name)')
    ...
    const row: Record<string, unknown> = {
      ...
      causing_fighter_subtypes: causing.fighter_subtypes ?? [],
