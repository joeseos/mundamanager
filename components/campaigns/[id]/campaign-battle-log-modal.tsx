...
import {
  ...
  updateBattleLog,
  BattleLogParams,
} from '@/app/lib/campaigns/[id]/battle-logs';
...
interface Battle {
  ...
  attacker?: { gang_id?: string; gang_name: string };
  defender?: { gang_id?: string; gang_name: string };
  winner?: { gang_id?: string; gang_name: string };
}
...
type GangEntry = { id: number; gangId: string; role: GangRole };
...
const CampaignBattleLogModal = ({
  ...
  const populateFormWithBattleData = () => {
    ...
    if (matchingScenario) {
      ...
      setCustomScenario(
        battleToEdit.scenario || battleToEdit.scenario_name || ''
      );
    }
    ...
    if (
      ...
      if (battleToEdit.attacker_id || battleToEdit.attacker?.gang_id) {
        const gangId =
          battleToEdit.attacker?.gang_id || battleToEdit.attacker_id || '';
        if (gangId) {
          newGangsInBattle.push({ id: idx++, gangId, role: 'attacker' });
        }
      }

      if (battleToEdit.defender_id || battleToEdit.defender?.gang_id) {
        const gangId =
          battleToEdit.defender?.gang_id || battleToEdit.defender_id || '';
        if (gangId) {
          newGangsInBattle.push({ id: idx++, gangId, role: 'defender' });
        }
      }
    }
  ...
  useEffect(() => {
    ...
    if (
      ...
      gangsInBattle.every((gang) => !gang.gangId)
    ) {
      return;
    }
    ...
    if (winner && winner !== 'draw') {
      ...
      newAvailableTerritories = territories.filter(
        (territory) =>
          ...
          losingGangIds.includes(territory.controlled_by)
      );
    }

    // Compare by value using JSON.stringify to avoid infinite loops
    const currentTerritoriesJSON = JSON.stringify(
      availableTerritories.map((t) => t.id).sort()
    );
    const newTerritoriesJSON = JSON.stringify(
      newAvailableTerritories.map((t) => t.id).sort()
    );
  ...
  const handleGangRoleChange = (gangEntryId: number, newRole: GangRole) => {
    setGangsInBattle(
      gangsInBattle.map((entry) =>
        entry.id === gangEntryId ? { ...entry, role: newRole } : entry
      )
    );
  };
  ...
  const handleSaveBattle = async () => {
    ...
    const attackers = gangsInBattle.filter(
      (g) => g.role === 'attacker' && g.gangId
    );
    const defenders = gangsInBattle.filter(
      (g) => g.role === 'defender' && g.gangId
    );
    const gangsWithRole = gangsInBattle.filter(
      (g) => g.role !== 'none' && g.gangId
    );

    // If any gang has a role, ensure both attackers and defenders exist
    if (
      gangsWithRole.length > 0 &&
    ...
    if (!winner) {
      toast({ variant: 'destructive', description: 'Please select a winner' });
      return false;
    }
    ...
    try {
      ...
      const participants = gangsInBattle
        ...
        .map((gang) => ({ role: gang.role, gang_id: gang.gangId }));
      ...
      if (isEditMode && battleToEdit) {
        ...
        toast({ description: 'Battle report updated successfully' });
      } else {
        await createBattleLog(campaignId, battleData);
        toast({ description: 'Battle report added successfully' });
      }
      ...
      console.error(
        isEditMode
          ? 'Error updating battle report:'
          : 'Error creating battle report:',
        error
      );
      toast({
        variant: 'destructive',
        description: isEditMode
          ? 'Failed to update battle report'
          : 'Failed to create battle report',
      });
      return false;
    }
  };
          ...
          <div className="space-y-4">
            ...
            {gangsInBattle.map((gangEntry) => {
              ...
              const availableGangsForThisEntry = availableGangs.filter(
                (gang) =>
                  !selectedGangs.includes(gang.id) ||
                  gang.id === gangEntry.gangId
              );
                ...
                <div key={gangEntry.id} className="grid gap-3">
                  <div className="flex items-center gap-3">
                    <select
                      ...
                      onChange={(e) =>
                        handleGangChange(gangEntry.id, e.target.value)
                      }
                      disabled={isLoadingBattleData}
                    >
                  ...
                  <div className="flex items-center gap-4 ml-2">
                    <div className="flex items-center space-x-2">
                      <input
                        ...
                        onChange={() =>
                          handleGangRoleChange(gangEntry.id, 'attacker')
                        }
                        disabled={isLoadingBattleData || !gangEntry.gangId}
                        className="h-4 w-4 text-black focus:ring-black border-gray-300"
                      />
                      <Label
                        htmlFor={`role-${gangEntry.id}-attacker`}
                        className="text-sm"
                      >
                        Attacker
                      </Label>
                    ...
                    <div className="flex items-center space-x-2">
                      <input
                        ...
                        onChange={() =>
                          handleGangRoleChange(gangEntry.id, 'defender')
                        }
                        disabled={isLoadingBattleData || !gangEntry.gangId}
                        className="h-4 w-4 text-black focus:ring-black border-gray-300"
                      />
                      <Label
                        htmlFor={`role-${gangEntry.id}-defender`}
                        className="text-sm"
                      >
                        Defender
                      </Label>
                    ...
                    <div className="flex items-center space-x-2">
                      <input
                        ...
                        onChange={() =>
                          handleGangRoleChange(gangEntry.id, 'none')
                        }
                        disabled={isLoadingBattleData || !gangEntry.gangId}
                        className="h-4 w-4 text-black focus:ring-black border-gray-300"
                      />
                      <Label
                        htmlFor={`role-${gangEntry.id}-none`}
                        className="text-sm"
                      >
                        None
                      </Label>
            ...
            <div>
              ...
              >
                ...
                {availableTerritories.map((territory) => {
                  const controlledBy = getGangName(
                    territory.controlled_by || ''
                  );
                  return (
                    <option key={territory.id} value={territory.id}>
                      {territory.name}
                      {controlledBy && ` (currently held by ${controlledBy})`}
                    </option>
                  );
                })}
              </select>
