...
interface AdminTradingPostProps {
  ...
  setSelectedTradingPosts: (
    tradingPosts: string[] | ((prev: string[]) => string[])
  ) => void;
...
export function AdminTradingPost({
  ...
  const [tradingPostTypes, setTradingPostTypes] =
    useState<TradingPostType[]>(propTradingPostTypes);
  ...
  const handleSave = () => {
    toast({
      description:
        'Trading Post selections saved. Remember to update the equipment to apply changes.',
      variant: 'default',
    });
    return true;
  };
  ...
  const handleTradingPostToggle = (tradingPostId: string, checked: boolean) => {
    if (checked) {
      ...
      setSelectedTradingPosts((prev) =>
        prev.filter((id) => id !== tradingPostId)
      );
    }
  };
        ...
        <div className="mt-4 p-3 bg-gray-50 rounded-lg">
          ...
          <div className="flex flex-wrap gap-2">
            {selectedTradingPosts.map((tradingPostId) => {
              const tradingPost = tradingPostTypes.find(
                (tp) => tp.id === tradingPostId
              );
              return tradingPost ? (
                <span
                  key={tradingPostId}
