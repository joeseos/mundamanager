...
BEGIN
    ...
    -- resolved to an id once here, as get_fighter_available_advancements does.
            ...
            jsonb_agg(
                jsonb_build_object(
                    ...
                    'available_acquisition_types', CASE
                        ...
                        -- Regular skill costs. The subtype list is an N23 rule: it names the
                        -- subtypes that pay these prices, omitting Gangers and Exotic Beasts,
                        -- who buy at flat cost off their own table. An edition that earns
                        -- Advancements by rank applies one table to every subtype and prices
                        -- the result itself, so it has no such split and reads this array
                        -- only as the catalog of acquisition type ids.
