...
function normalizeEquipmentSelection(
  ...
  if (
    ...
    ['optional', 'optional_single', 'single', 'multiple'].some(
      (k) => k in equipmentSelection
    )
  ) {
    const result: EquipmentSelection = {};
    let idCounter = 0;

    (['optional', 'optional_single', 'single', 'multiple'] as const).forEach(
      (selectType) => {
        ...
        if (typeGroup && typeof typeGroup === 'object') {
          (['weapons', 'wargear'] as const).forEach((categoryName) => {
            ...
            if (Array.isArray(categoryData) && categoryData.length > 0) {
              ...
              const isNestedArrays =
                categoryData.length > 0 && Array.isArray(categoryData[0]);
              ...
              if (isNestedArrays) {
                ...
                categoryData.forEach((group: any[], groupIndex: number) => {
                  if (Array.isArray(group) && group.length > 0) {
                    ...
                    if (
                      selectType === 'optional' ||
                      selectType === 'optional_single'
                    ) {
                      // For optional types, separate defaults and replacements
                      const defaults = group.filter(
                      ...
                      defaults.forEach((defaultItem: any) => {
                        if (
                          defaultItem.replacements &&
                          Array.isArray(defaultItem.replacements)
                        ) {
                          defaultItem.replacements.forEach(
                            (replacement: any) => {
                              allReplacements.push({
                                ...
                                equipment_category:
                                  replacement.equipment_category,
                ...
                if (
                  ...
                  // For optional types, separate defaults and replacements
                  const defaults = categoryData.filter(
                    (item: any) => item.is_default
                  );
                  const allReplacements: GangEquipmentOption[] = [];
...
export default function GangAdditions({
  ...
  const [selectedGangAdditionTypeId, setSelectedGangAdditionTypeId] =
    useState('');
  const [selectedGangAdditionClass, setSelectedGangAdditionClass] =
    useState<string>('');
  ...
  const [defaultEquipmentNames, setDefaultEquipmentNames] = useState<
    Record<string, string>
  >({});
  const [fetchError, setFetchError] = useState<string | null>(null);
  const [useBaseCostForRating, setUseBaseCostForRating] =
    useState<boolean>(true);
  const [isAdding, setIsAdding] = useState(false);
  const [selectedSubTypeId, setSelectedSubTypeId] = useState('');
  const [availableSubTypes, setAvailableSubTypes] = useState<
    Array<{ id: string; sub_type_name: string }>
  >([]);

  // Add state to track selected equipment with costs
  const [selectedEquipment, setSelectedEquipment] = useState<
    Array<{ equipment_id: string; cost: number; quantity: number }>
  >([]);
  ...
  useEffect(() => {
    if (availableSubTypes.length > 0 && !selectedSubTypeId) {
      ...
      if (defaultSubType) {
        ...
        const cheapestSubType = availableSubTypes.reduce((lowest, current) => {
          const lowestCost =
            gangAdditionTypes.find((ft) => ft.id === lowest.id)?.total_cost ??
            Infinity;
          const currentCost =
            gangAdditionTypes.find((ft) => ft.id === current.id)?.total_cost ??
            Infinity;
  ...
  const fetchGangAdditionTypes = async () => {
    try {
      const { getGangAdditionTypesUncachedClient } = await import(
  ...
  const handleGangAdditionClassChange = (
  ...
  const handleGangAdditionTypeChange = (
    e: React.ChangeEvent<HTMLSelectElement>
  ) => {
    ...
    if (typeId) {
      ...
      const fighterTypeGroup = gangAdditionTypes.filter(
        (t) =>
  ...
  const handleSubTypeChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (subTypeId) {
      ...
      const mainType = gangAdditionTypes.find(
        (t) => t.id === selectedGangAdditionTypeId
      );
      if (mainType) {
  ...
  const renderEquipmentSelection = () => {
    const selectedType = gangAdditionTypes.find(
      (t) => t.id === selectedGangAdditionTypeId
    );
    if (!selectedType?.equipment_selection) return null;

    console.log(
      'Original equipment_selection:',
      selectedType.equipment_selection
    );

    // Normalize equipment_selection to UI format
    const normalizedSelection = normalizeEquipmentSelection(
      selectedType.equipment_selection
    );
        ...
        {allCategories.map(([categoryId, categoryData]) => {
          ...
          if (categoryData.options && Array.isArray(categoryData.options)) {
            categoryData.options.forEach((option: GangEquipmentOption) => {
              ...
              const equipCategoryName =
                optionAny.equipment_category ||
                inferCategoryFromEquipmentName(
                  optionAny.equipment_name || categoryName
                );
            ...
            <div key={categoryId} className="space-y-3">
              {categoryData.default && categoryData.default.length > 0 && (
                <div className="space-y-1">
                  ...
                  <div className="space-y-1">
                    {categoryData.default.map(
                      (item: EquipmentDefaultItem, index: number) => {
                        // Access equipment_name with type assertion for safety
                        const defaultItem = item as any;
                        const equipmentName =
                          defaultItem.equipment_name || 'Equipment';
              ...
              {categoryData.options && categoryData.options.length > 0 && (
                <div className="space-y-1">
                  <label className="block text-sm font-medium text-gray-700">
                    {isOptional
                      ? `Optional ${categoryName} (Replaces one default)`
                      : isOptionalSingle
                        ? `Optional ${categoryName} (Choose one replacement)`
                        : isSingle
                          ? `Select ${categoryName} (Choose one)`
                  ...
                  <div className="space-y-1">
                    ...
                    {isOptionalSingle && (
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          checked={
                            !categoryData.options?.some((o: any) =>
                          ...
                          onChange={() => {
                            ...
                            setSelectedEquipmentIds((prev) => {
                              ...
                              return prev.filter(
                                (id) =>
                                  !currentCategoryOptions.some(
                                    (o: any) => `${categoryId}-${o.id}` === id
                                  )
                              );
                            });
                            ...
                            setFighterCost((prevCost) => {
                              ...
                              const prevSelectedCost = prevSelectedUniqueId
                                ? currentCategoryOptions.find(
                                    (o: any) =>
                                      `${categoryId}-${o.id}` ===
                                      prevSelectedUniqueId
                                  )?.cost || 0
                                : 0;
                              return String(
                                parseInt(prevCost || '0') - prevSelectedCost
                              );
                            });
                          }}
                        />
                        <label
                          htmlFor={`${categoryId}-keep-default`}
                          className="text-sm font-medium"
                        >
                          Keep Default{' '}
                          {categoryData.default?.[0]?.equipment_name
                            ? `(${categoryData.default[0].equipment_name})`
                            : 'Equipment'}
                        </label>
                      </div>
                    ...
                    {sortedCategories
                      .flatMap((category) => categorizedOptions[category])
                      .sort((a, b) => {
                        // Sort alphabetically
                              ...
                              <input
                                ...
                                onChange={() => {
                                  ...
                                  setSelectedEquipment((prev) => {
                                    ...
                                    const currentCategoryOptions =
                                      categoryData.options || [];
                                    const previouslySelectedInThisCategory =
                                      selectedEquipmentIds.filter((id) =>
                                        currentCategoryOptions.some(
                                          (o: any) =>
                                            `${categoryId}-${o.id}` === id
                                        )
                                    ...
                                    let filtered = prev.filter((item) => {
                                      ...
                                      const wasSelectedFromThisCategory =
                                        previouslySelectedInThisCategory.some(
                                          (selectedId) => {
                                            const equipmentIdFromSelected =
                                              selectedId.split('-').pop();
                                            return (
                                              equipmentIdFromSelected ===
                                              item.equipment_id
                                            );
                                          }
                                        );
                              ...
                              <Checkbox
                                ...
                                onCheckedChange={(checked) => {
                                  ...
                                  if (checked === true) {
                                    ...
                                    const isReplacement =
                                      categoryData.select_type === 'optional' ||
                                      categoryData.select_type ===
                                        'optional_single';
                                    if (
                                      isReplacement &&
                                      ...
                                      const defaultItem = categoryData
                                        .default[0] as any;
                                    ...
                                    if (
                                      ...
                                      setSelectedEquipment((prev) => {
                                        ...
                                        const withoutReplacement = prev.filter(
                                          (item) =>
                                            item.equipment_id !== option.id
                                        );

                                        // Check if default item exists, if so increase its quantity, otherwise add it back
                                        const existingDefaultIndex =
                                          withoutReplacement.findIndex(
                                            (item) =>
                                              item.equipment_id ===
                                              defaultItem.id
                                          );
  ...
  const calculateTotalCostWithEquipment = () => {
    ...
    return { manualCost, equipmentCost, totalCost: manualCost + equipmentCost };
  };
  ...
  const getDefaultEquipment = (
    ...
    Object.entries(normalizedSelection).forEach(
  ...
  useEffect(() => {
    if (selectedGangAdditionTypeId) {
      ...
      if (selectedType?.equipment_selection) {
        ...
        const defaultCost = defaultEquipment.reduce(
  ...
  const handleAddFighter = async () => {
    ...
    try {
      ...
      const gangAdditionTypeForEquipment = gangAdditionTypes.find(
        (t) => t.id === fighterTypeIdToUse
      );
      const defaultEquipment =
        gangAdditionTypeForEquipment?.default_equipment?.map((item) => ({
      ...
      const newFighter = {
        ...
        skills: data.skills
          ? data.skills.reduce((acc: FighterSkills, skill: any) => {
      ...
      setFetchError(
        error instanceof Error ? error.message : 'Failed to add fighter'
      );
      setIsAdding(false);
      return false;
    }
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        >
          ...
          {((): React.ReactElement[] => {
            const nonAlliances = gangAdditionTypes.filter(
              (t) => !t.alliance_id
            );
            ...
            const groupLabelConfig = [
              ...
              {
                label: 'Alliances: Criminal Organisations',
                maxRank: 49,
                alliance: true,
              },
              {
                label: 'Alliances: Merchant Guilds',
                maxRank: 59,
                alliance: true,
              },
            ...
            const nonAllianceGroups = nonAlliances.reduce(
              (groups, type) => {
                const classType = type.fighter_class;
                const rank =
                  gangAdditionRank[classType.toLowerCase()] ?? Infinity;
            ...
            const allianceGroups = alliances.reduce(
              (groups, type) => {
                const crewName =
                  type.alliance_crew_name || 'Unnamed Delegation';
                const rank =
                  gangAdditionRank[crewName.toLowerCase()] ?? Infinity;
              ...
              .map(([groupLabel, classSet]) => (
                <optgroup key={groupLabel} label={groupLabel}>
                  {Array.from(classSet)
                    .sort((a, b) => {
                      const rankA =
                        gangAdditionRank[a.toLowerCase()] ?? Infinity;
                      const rankB =
                        gangAdditionRank[b.toLowerCase()] ?? Infinity;
                      return rankA - rankB;
                    })
                    .map((classType) => (
          ...
          {((): React.ReactElement[] => {
            ...
            return Array.from(typeClassMap.values())
              .sort((a, b) => {
                ...
                const alignmentA =
                  a.fighter.alignment?.toLowerCase() ?? 'unaligned';
                const alignmentB =
                  b.fighter.alignment?.toLowerCase() ?? 'unaligned';
                ...
                return a.fighter.fighter_type.localeCompare(
                  b.fighter.fighter_type
                );
              })
              .map(({ fighter, cost }) => {
          ...
          >
            ...
            {[...availableSubTypes]
              .sort((a, b) => {
                ...
                const aCost =
                  gangAdditionTypes.find((ft) => ft.id === a.id)?.total_cost ??
                  0;
                const bCost =
                  gangAdditionTypes.find((ft) => ft.id === b.id)?.total_cost ??
                  0;
              ...
              .map((subType) => {
                const subTypeCost =
                  gangAdditionTypes.find((ft) => ft.id === subType.id)
                    ?.total_cost ?? 0;
                const lowestSubTypeCost = Math.min(
                  ...availableSubTypes.map(
                    (sub) =>
                      gangAdditionTypes.find((ft) => ft.id === sub.id)
                        ?.total_cost ?? Infinity
                  )
                );
      ...
      <div className="flex items-center space-x-2 mb-4 mt-2">
        ...
        <div className="relative group">
          ...
          <div className="absolute bottom-full mb-2 hidden group-hover:block bg-black text-white text-xs p-2 rounded w-72 -left-36 z-50">
            When enabled, the fighter&apos;s rating is calculated using their
    ...
    <Modal
      ...
      confirmDisabled={
        ...
        (() => {
          ...
          const normalizedSelection = selectedType?.equipment_selection
            ? normalizeEquipmentSelection(selectedType.equipment_selection)
            : {};
          ...
          for (const [categoryId, categoryData] of Object.entries(
            ...
            if (
              ...
              const selectedFromCategory = selectedEquipmentIds.some((id) =>
                categoryData.options?.some(
                  (opt: GangEquipmentOption) => `${categoryId}-${opt.id}` === id
                )
              );
