...
export async function GET(request: Request) {
  try {
    ...
    const { data: fighters, error: fightersError } = await supabase
      .from('fighters')
      .select('id, fighter_name, fighter_type, fighter_class, fighter_classes, gang_id')
      .in('gang_id', gangIds)
    ...
    const fightersByGang = new Map<string, Array<{ id: string; fighter_name: string; fighter_type: string | null; fighter_class: string | null; fighter_classes?: string[] | null; gang_id: string }>>();
