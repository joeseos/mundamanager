  ...
  try {
    if (id) {
      ...
      if (availabilitiesError) {
        console.warn(
          'Error fetching from equipment_availability:',
          availabilitiesError
        );
      }
      ...
      const { data: tradingPostAssociations, error: tradingPostError } =
        await supabase
      ...
      if (tradingPostError) {
        console.warn(
          'Error fetching trading post associations:',
          tradingPostError
        );
      }

      console.log(
        'Fetched trading post associations:',
        tradingPostAssociations || []
      );
      ...
      const formattedAdjustedCosts = (
        (adjustedCosts as AdjustedCostData[]) || []
      )
      ...
      const formattedAvailabilities = (
        (availabilities as AvailabilityData[]) || []
      )
      ...
      const tradingPostIds = (tradingPostAssociations || []).map(
        (tp) => tp.trading_post_type_id
      );
      console.log('Trading post IDs:', tradingPostIds);

      // Fetch trading post types for the component to display names
      const { data: tradingPostTypes, error: tradingPostTypesError } =
        await supabase
      ...
      if (tradingPostTypesError) {
        console.warn(
          'Error fetching trading post types:',
          tradingPostTypesError
        );
      }
      ...
      try {
        ...
        if (categoriesError) {
          console.warn(
            'Error fetching fighter effect categories:',
            categoriesError
          );
        } else {
      ...
      try {
        ...
        const {
          data: equipmentFighterTypes,
          error: equipmentFighterTypesError,
        } = await supabase
        ...
        if (equipmentFighterTypesError) {
          console.warn(
            'Error fetching fighter types with equipment:',
            equipmentFighterTypesError
          );
        } else {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (
      equipment_type.toLowerCase() === 'weapon' &&
      weapon_profiles &&
      weapon_profiles.length > 0
    ) {
      const weaponId = equipment.id;
      const cleanedWeaponProfiles = weapon_profiles.map(
        (profile: WeaponProfile) => ({
          ...
          range_short:
            profile.range_short === null ? '' : profile.range_short || '',
          range_long:
            profile.range_long === null ? '' : profile.range_long || '',
    ...
    if (gang_adjusted_costs && gang_adjusted_costs.length > 0) {
      const adjustedCostRecords = gang_adjusted_costs.map(
        (adjusted_cost: GangAdjustedCost) => ({
          equipment_id: equipment.id,
    ...
    if (equipment_availabilities && equipment_availabilities.length > 0) {
      const availabilityRecords = equipment_availabilities.map(
      ...
      if (availabilityError) {
        console.warn(
          'Error inserting into equipment_availability:',
          availabilityError
        );
      }
    }
...
export async function PUT(request: Request) {
  ...
  try {
    ...
    if (fighter_types !== undefined) {
      ...
      if (fetchError) {
        console.error(
          'Error fetching current fighter type associations:',
          fetchError
        );
      ...
      if (currentAssociations) {
        ...
        const hasChanges =
          ...
          JSON.stringify([...fighter_types].sort());
        ...
        if (hasChanges) {
          ...
          console.log(
            'No changes to fighter type associations, preserving current data'
          );
        }
      } else {
        console.log(
          `fighter_types not provided, preserving existing associations for ID: ${id}`
        );
      }
    }
    ...
    if (gang_adjusted_costs) {
      ...
      if (gang_adjusted_costs.length > 0) {
        ...
        const gangTypeMap = new Map(
          (gangTypes as GangTypeData[]).map((gt) => [
            gt.gang_type,
            gt.gang_type_id,
          ])
        );
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    if (equipment_availabilities !== undefined) {
      ...
      if (deleteError) {
        console.warn(
          'Error deleting from equipment_availability:',
          deleteError
        );
      }
      ...
      if (
        Array.isArray(equipment_availabilities) &&
        equipment_availabilities.length > 0
      ) {
        const availabilityRecords = equipment_availabilities.map(
          (avail: EquipmentAvailability) => ({
            equipment_id: id,
    ...
    if (fighter_effects !== undefined) {
      ...
      if (Array.isArray(fighter_effects)) {
        ...
        console.log('Existing effects query result:', {
          existingEffects,
          fetchError,
        });
        ...
        if (!existingEffects || existingEffects.length === 0) {
          ...
          if (!allEffectsError && allEffects) {
            ...
            console.log(
              'Alternative query found effects:',
              finalExistingEffects?.length || 0
            );
          }
        }
        ...
        if (finalExistingEffects && finalExistingEffects.length > 0) {
          ...
          const currentEffectIds = new Set(
            fighter_effects.map((effect) => effect.id)
          );
          console.log(
            'Current effect IDs from request:',
            Array.from(currentEffectIds)
          );

          // Find effects to delete (existing effects not in the current list)
          const effectsToDelete = (finalExistingEffects || [])
            .filter(
              (existing) => existing.id && !currentEffectIds.has(existing.id)
            )
          ...
          if (effectsToDelete.length > 0) {
            ...
            for (const effectId of effectsToDelete) {
              ...
              if (deleteModifiersError) {
                console.error(
                  'Error deleting modifiers for effect',
                  effectId,
                  ':',
                ...
                console.log(
                  'Successfully deleted modifiers for effect:',
                  effectId
                );
              }
            }
            ...
            fighter_effects.length === 0 &&
            finalExistingEffects &&
            finalExistingEffects.length > 0
          ) {
            ...
            const allEffectIds = finalExistingEffects.map(
              (effect) => effect.id
            );
            ...
            if (deleteAllEffectsError) {
              console.error(
                'Error deleting all effects:',
                deleteAllEffectsError
              );
            } else {
              console.log(
                'Successfully deleted all effects for equipment:',
                id
              );
            }
          } else {
            console.log(
              'No effects to delete. fighter_effects.length:',
              fighter_effects.length,
        ...
        for (const effect of fighter_effects) {
          ...
          if (isNewEffect) {
            ...
            if (effect.modifiers && effect.modifiers.length > 0) {
              const modifiersToCreate = effect.modifiers.map(
                (modifier: any) => ({
              ...
              if (createModifiersError) {
                console.error(
                  'Error creating modifiers:',
                  createModifiersError
                );
              }
            }
          } else {
            ...
            if (effect.modifiers) {
              ...
              const { data: existingModifiers, error: fetchModifiersError } =
                await supabase
                  .from('fighter_effect_type_modifiers')
                  .select('id')
              ...
              if (fetchModifiersError) {
                console.error(
                  'Error fetching existing modifiers:',
                  fetchModifiersError
                );
              } else {
                ...
                const currentModifierIds = new Set(
                  effect.modifiers
                    .filter(
                      (mod: any) => mod.id && mod.id.indexOf('temp-') !== 0
                    )
                    .map((mod: any) => mod.id)
                );
                ...
                const modifiersToDelete = (existingModifiers || [])
                  .filter(
                    (existing) =>
                      existing.id && !currentModifierIds.has(existing.id)
                  )
              ...
              for (const modifier of effect.modifiers) {
                const isNewModifier =
                  !modifier.id || modifier.id.indexOf('temp-') === 0;
