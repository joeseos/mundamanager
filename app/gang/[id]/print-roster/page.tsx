 ...
 * (copied from the main gang page so roster ordering matches exactly)
 */
async function initializeOrFixPositioning(
  ...
  if (Object.keys(pos).length === 0) {
    ...
    Object.entries(pos as Record<string, string>).forEach(
      ([position, fighterId]) => {
    ...
    for (const [position, fighterId] of Object.entries(validPositions)) {
      newPositioning[positionMapping[Number(position)] ?? expectedPosition++] =
        fighterId;
    }
    pos = newPositioning;
