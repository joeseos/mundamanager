...
export default function MembersTable({
  ...
  const fetchUserGangs = async (userId: string) => {
    try {
      ...
      const { data: currentCampaignGangs, error: currentCampaignError } =
        await supabase
          .from('campaign_gangs')
          .select('gang_id')
      ...
      toast({ variant: 'destructive', description: 'Failed to load gangs' });
    }
  };
  ...
  const handleAddGang = async () => {
    ...
    try {
      ...
      toast({ description: `Added ${selectedGang.name} to the campaign` });
      ...
      toast({
        variant: 'destructive',
        description:
          error instanceof Error ? error.message : 'Failed to add gang',
      });
      return false;
    }
  };
  ...
  const handleRemoveGang = async () => {
    ...
    try {
      ...
      onMemberUpdate({ removedGangIds: [gangToRemove.gangId] });
      toast({
      ...
      toast({
        variant: 'destructive',
        description:
          error instanceof Error ? error.message : 'Failed to remove gang',
      });
      return false;
    }
  };
    ...
    () => (
      <div className="space-y-4">
        <p className="text-sm text-gray-600">
          Select a gang to add to the campaign:
        </p>
        ...
        {userGangs.length === 0 && (
          <p className="text-sm text-gray-500 text-center">
            No gangs available for this player.
          </p>
        )}
      </div>
    ),
    ...
    () => (
      <div className="space-y-4">
        <p className="text-sm text-gray-600">
          ...
          <span className="font-medium">{roleChange?.username}</span>&apos;s
          role from{' '}
      ...
      <div className="space-y-4">
        <p className="text-sm text-gray-600">
          ...
          <span className="font-medium">
            {memberToRemove?.profile.username}
          </span>{' '}
          from this campaign?
          {memberToRemove?.gangs[0] && (
            <span className="block mt-2 text-red-600">
              This will also remove their gang &quot;
              {memberToRemove.gangs[0].gang_name}&quot; from the campaign.
            </span>
          )}
        </p>
  ...
  const removeGangModalContent = useMemo(
    () => (
      <div className="space-y-4">
        <p className="text-sm text-gray-600">
          Are you sure you want to remove{' '}
          <span className="font-medium">{gangToRemove?.gangName}</span> from
          this campaign?
        </p>
      </div>
    ),
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 border-b">
              <th className="px-4 py-2 text-left font-medium max-w-[8rem]">
                Gang
              </th>
              <th className="px-2 py-2 text-left font-medium max-w-[5rem]">
                Type
              </th>
              <th className="px-2 py-2 text-left font-medium max-w-[3rem]">
                Player
              </th>
              <th className="px-2 py-2 text-left font-medium max-w-[3.5rem]">
                Role
              </th>
              <th className="px-2 py-2 text-right font-medium max-w-[2rem]">
                Rating
              </th>
              <th className="px-2 py-2 text-right font-medium max-w-[3rem]">
                Reputation
              </th>
              {isAdmin && (
                <th className="px-2 py-2 text-right font-medium max-w-[2.5rem]">
                  Action
                </th>
              )}
            </tr>
            ...
            <div className="flex justify-between items-start mb-2">
              <div>
                {member.gangs[0]?.gang_name ? (
                  <div className="flex items-center gap-1">
                    <span
                      className="inline-flex items-center px-2.5 py-0.5 rounded-full text-small font-semibold bg-gray-100"
                      style={{
                        color: member.gangs[0]?.gang_colour || '#000000',
                      }}
                    >
