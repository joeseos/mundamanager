      ...
      <div className="space-y-4">
        {allCategories.map(([categoryId, categoryData]) => {
          ...
          const usedSlots = isOptional
            ? (categoryData.options || []).filter(o => selectedEquipmentIds.includes(`${categoryId}-${o.id}`)).length : 0;
          const remainingSlots = totalSlots - usedSlots;
          const strictSelectedId = isOptional && replacementMode === 'strict'
            ? selectedEquipmentIds.find(id => (categoryData.options || []).some((o: any) => `${categoryId}-${o.id}` === id))
            : undefined;
              ...
              {categoryData.options && categoryData.options.length > 0 && (
                <div className="space-y-1">
                  ...
                  <div className="space-y-1">
                    ...
                    {/* Keep Default radio for strict optional mode */}
                              ...
                              onCheckedChange={(checked) => {
                                ...
                                setSelectedEquipmentIds(prev => {
                                  ...
                                  setSelectedEquipment(prevEquip => {
                                    const currentCategoryOptions = categoryData.options || [];
                                    let filtered = prevEquip.filter(item => item.categoryId !== categoryId);
                          ...
                          <div key={uniqueOptionId} className="flex items-center gap-2">
                            <input
                              ...
                              onChange={() => {
                                ...
                                // Remove defaults + previous, add new with totalSlots quantity
                                setSelectedEquipment((prev) => {
                                  let filtered = prev.filter(item => item.categoryId !== categoryId);
                                  return [...filtered, {
                                    categoryId: categoryId,
      ...
      if (result.data?.created_beasts && result.data.created_beasts.length > 0) {
        result.data.created_beasts.forEach(beast => {
          const beastFighter = {
            ...
            // Process skills
            skills: beast.skills ? beast.skills.reduce((acc: FighterSkills, skill: any) => {
              acc[skill.skill_name] = {
                id: skill.skill_id,
                credits_increase: 0,
                xp_cost: 0,
