...
export default function GangVehicles({
  ...
  const [editingVehicle, setEditingVehicle] =
    useState<CombinedVehicleProps | null>(null);
  ...
  const crewFighters = fighters.filter(
    (fighter) => fighter.fighter_class === 'Crew'
  );
  ...
  const allVehicles = useMemo<CombinedVehicleProps[]>(() => {
    ...
    const fighterVehicles = fighters.flatMap((fighter) =>
      (fighter.vehicles || []).map(
        (vehicle) =>
          ({
            ...vehicle,
  ...
  const handleMoveToFighter = async () => {
    ...
    try {
      ...
      const selectedFighterData = fighters.find(
        (f) => f.id === selectedFighter
      );
      ...
      if (data.removed_from) {
        const vehiclesWithSwapped = [
          ...updatedVehicles,
          { ...data.removed_from, gang_id: gangId, equipment: [] },
        ];
      ...
      if (deletingVehicle && deletingVehicle.assigned_to && onFighterUpdate) {
        const originalFighter = originalFighters.find(
          (f) => f.fighter_name === deletingVehicle.assigned_to
        );
        if (originalFighter) {
      ...
      toast({
        ...
        description:
          error instanceof Error
            ? error.message
            : 'Failed to move vehicle to fighter',
        variant: 'destructive',
      });
    } finally {
      setIsLoading(false);
  ...
  const handleRemoveSpecialRule = (ruleToRemove: string) => {
    setVehicleSpecialRules((prev) =>
      prev.filter((rule) => rule !== ruleToRemove)
    );
  };
    ...
    try {
      ...
      if (editingVehicle.assigned_to && onFighterUpdate) {
        const fighter = fighters.find(
          (f) => f.fighter_name === editingVehicle.assigned_to
        );
      ...
      if (!responseData.updatedSpecialRules && vehicleSpecialRules.length > 0) {
        ...
        console.warn(
          'Warning: Special rules may not have been updated on the server'
        );
      }
  ...
  const handleDeleteVehicle = async () => {
    ...
    try {
      ...
      if (!isUnassigned && onFighterUpdate) {
        updatedFighter = fighters.find(
          (f) => f.fighter_name === deletingVehicle.assigned_to
        );
        if (updatedFighter) {
          const fighterWithoutVehicle = {
            ...updatedFighter,
            vehicles: (updatedFighter.vehicles || []).filter(
              (v) => v.id !== deletingVehicle.id
            ),
          };
      ...
      const response = await fetch(`/api/gangs/${gangId}/vehicles`, {
        ...
        body: JSON.stringify({ vehicleId: deletingVehicle.id }),
      });
            ...
            <div className="px-0">
              <div className="border-t pt-4">
                ...
                <Button
                  ...
                  disabled={
                    selectedVehicle === null || !selectedFighter || isLoading
                  }
                  className="w-full"
                >
                  Move to Fighter
        ...
        >
          <div className="space-y-4">
            <p>
              ...
              {deletingVehicle.vehicle_name || deletingVehicle.vehicle_type}
              &quot;?
            </p>
