...
RETURNS TABLE (
    ...
    -- This pet's exotic_beasts equipment is a fighter_defaults row on another fighter type.
    is_granted_with_fighter boolean,
    -- This fighter type is a pet associated with a dramatis personae.
    is_associated_pet boolean,
    -- Dramatis fighter type that grants this pet. Null when is_associated_pet is false.
    associated_pet_owner_id uuid
    ...
    SELECT
        ...
        (
            SELECT COALESCE(jsonb_agg(
                jsonb_build_object(
                    'id', e.id,
                    -- default_id and target_fighter_default_id let the hire preview show a
                    -- weapon accessory under the default weapon it is fitted to.
                    'default_id', fd.id,
