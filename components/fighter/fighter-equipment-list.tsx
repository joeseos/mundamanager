...
export function WeaponList({
  ...
  const weapons = sortedEquipment.filter(
    (item) => item.equipment_type === 'weapon'
  );
  const wargear = sortedEquipment.filter(
    (item) => item.equipment_type === 'wargear'
  );
  const vehicleUpgrades = sortedEquipment.filter(
    (item) => item.equipment_type === 'vehicle_upgrade'
  );

  const renderRow = (item: Equipment) => (
    <tr
      key={
        item.fighter_equipment_id ||
        `${item.equipment_id}-${item.equipment_name}`
      }
      className="border-b"
    >
      <td className="px-1 py-1">{item.equipment_name}</td>
                ...
                <>
                  ...
                  {vehicleUpgrades.length > 0 && weapons.length > 0 && (
                    <tr>
                      <td
                        colSpan={3}
                        className="border-t-8 border-gray-100 p-0"
                      />
                    </tr>
                  )}
                  {vehicleUpgrades.map(renderRow)}
                  {wargear.length > 0 &&
                    (weapons.length > 0 || vehicleUpgrades.length > 0) && (
                      <tr>
                        <td
                          colSpan={3}
                          className="border-t-8 border-gray-100 p-0"
