...
interface FighterCardProps
  extends Omit<
    FighterProps,
    'fighter_name' | 'fighter_type' | 'vehicles' | 'skills' | 'effects'
  > {
...
const FighterCard = memo(function FighterCard({
    ...
    return vehicle.equipment
      ...
      .map((weapon) => ({
        fighter_weapon_id:
          ...
          weapon.vehicle_weapon_id ||
          weapon.equipment_id,
        weapon_id: weapon.equipment_id,
        weapon_name:
          weapon.is_master_crafted || weapon.master_crafted
            ? `${weapon.equipment_name} (Master-crafted)`
            : weapon.equipment_name,
        weapon_profiles:
          weapon.weapon_profiles?.map((profile) => ({
            ...profile,
            ...
            is_master_crafted:
              (profile as any).is_master_crafted ||
              !!weapon.master_crafted ||
              !!weapon.is_master_crafted,
          })) || [],
        cost: weapon.cost,
  ...
  const stats = useMemo((): StatsType => {
    if (isCrew) {
      return {
        ...
        BS:
          adjustedStats.ballistic_skill === 0
            ? '-'
            : `${adjustedStats.ballistic_skill}+`,
      ...
      if (contentRef.current) {
        setTimeout(() => {
          ...
          const shouldBeMultiline =
            contentHeight > 24 && text.length > contentWidth / 8;
  ...
  const cardContent = (
    <div
      ...
      style={{
        backgroundImage:
          "url('https://res.cloudinary.com/dle0tkpbl/image/upload/v1747057650/fighter-card-background-4-web_pm2scj.avif')",
    ...
    >
            ...
            >
              ...
              <div className="flex flex-col items-baseline w-full">
                ...
                <div className="text-gray-300 text-xs sm:leading-5 sm:text-base overflow-hidden whitespace-nowrap print:text-gray-500">
                  ...
                  {fighter_sub_type && fighter_sub_type.fighter_sub_type
      ...
      {!isInactive && (
        <>
          ...
          <div
            className={`grid gap-y-2 mt-3 ${isMultiline ? 'grid-cols-[4.5rem,1fr]' : 'grid-cols-[6rem,1fr]'} print:gap-y-0`}
          >
            {isCrew && vehicle && (
              <>
                ...
                <div className="min-w-[0px] text-sm break-words">
                  {Array.isArray(vehicle?.special_rules)
                ...
                {vehicle.effects &&
                  ...
                  vehicle.effects['lasting damages'].length > 0 && (
                    <>
                      <div className="min-w-[0px] font-bold text-sm pr-4 whitespace-nowrap">
                        Damage
                      </div>
                        ...
                        {Object.entries(
                          vehicle.effects['lasting damages']
                            ...
                            .reduce<Record<string, number>>((acc, damage) => {
                              acc[damage.effect_name] =
                                (acc[damage.effect_name] || 0) + 1;
                              return acc;
                            }, {})
                        )
                          .map(([name, count]) =>
            ...
            {isCrew &&
              ...
              ((special_rules?.length ?? 0) > 0 ||
                (wargear?.length ?? 0) > 0 ||
                (advancements?.skills &&
                  Object.keys(advancements.skills).length > 0) ||
                free_skill) && (
                <>
                ...
                <div className="min-w-[0px] text-sm break-words">
                  {Object.entries(
                    wargear
                      ...
                      .reduce<Record<string, number>>((acc, item) => {
                        acc[item.wargear_name] =
                          (acc[item.wargear_name] || 0) + 1;
                        return acc;
                      }, {})
                  )
                    .map(([name, count]) =>
  ...
  return disableLink ? (
    ...
    <Link href={`/fighter/${id}`}>{cardContent}</Link>
  );
});
