...
export default function GangSearch({ campaignId }: GangSearchProps) {
  ...
  useEffect(() => {
    const loadCampaignGangs = async () => {
      try {
        ...
        const { data: campaignGangsData, error: campaignGangsError } =
          await supabase
            .from('campaign_gangs')
            .select('gang_id')
  ...
  const handleAddGang = async (
    gang: Database['public']['Tables']['gangs']['Row']
  ) => {
    try {
      const {
        data: { session },
                  ...
                  >
                    <div className="flex flex-col">
                      ...
                      <span className="text-sm text-gray-600">
                        {gang.gang_type} - Owner:{' '}
                        {gang.user?.username || 'Unknown'}
                      </span>
                    </div>
                  </button>
                </li>
