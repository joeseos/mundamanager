...
export interface UpdateGangParams {
  gang_id: string;
  name?: string;
  credits?: number;
...
export interface UpdateGangResult {
  gang_id: string;
  name: string;
  credits: number;
  reputation: number;
...
export async function updateGang(params: UpdateGangParams): Promise<ServerFunctionResult<UpdateGangResult>> {
  try {
    ...
    console.error('Error in updateGang server function:', error);
    return {
      success: false,
