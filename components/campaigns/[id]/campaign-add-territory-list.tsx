...
export default function TerritoryList({
  ...
  const [campaignTerritories, setCampaignTerritories] = useState<
    CampaignTerritory[]
  >(existingCampaignTerritories);
  ...
  const filteredTerritories = allTerritories
    ...
    .sort((a, b) => {
      const typeA =
        campaignTypes
          .find((ct) => ct.id === a.campaign_type_id)
          ?.campaign_type_name.toLowerCase() || '';
      const typeB =
        campaignTypes
          .find((ct) => ct.id === b.campaign_type_id)
          ?.campaign_type_name.toLowerCase() || '';
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mx-auto">
          {[...campaignTypes]
            .sort((a, b) => {
              const rankA =
                campaignRank[a.campaign_type_name.toLowerCase()] ?? Infinity;
              const rankB =
                campaignRank[b.campaign_type_name.toLowerCase()] ?? Infinity;
              return rankA - rankB;
            })
            .map((type) => (
          ...
          <tbody>
            {filteredTerritories.length === 0 ? (
              <tr>
                <td
                  colSpan={isAdmin ? 3 : 2}
                  className="px-4 py-2 text-gray-500 italic text-center"
                >
              ...
              filteredTerritories.map((territory) => {
                const type = campaignTypes.find(
                  (ct) => ct.id === territory.campaign_type_id
                );
                ...
                return (
                  <tr key={territory.id} className="border-b last:border-0">
                    ...
                    <td className="w-2/5 px-4 py-2 text-gray-500">
                      {typeName}
                    </td>
                    {isAdmin && (
