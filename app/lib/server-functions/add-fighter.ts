...
export interface AddFighterParams {
  fighter_name: string;
...
export interface AddFighterResult {
  fighter_id: string;
  fighter_name: string;
  fighter_type: string;
...
export async function addFighterToGang(params: AddFighterParams): Promise<ServerFunctionResult<AddFighterResult>> {
  try {
    ...
    if (fighterDefaultsData && fighterDefaultsData.length > 0) {
      const skillInserts = fighterDefaultsData.map((skill: any) => ({
        fighter_id: fighterId,
    ...
    // Update gang rating by fighter rating cost
    try {
    ...
    console.error('Error in addFighterToGang server function:', error);
    return {
      success: false,
